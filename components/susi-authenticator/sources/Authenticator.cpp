...
namespace Susi {
    ...
    Authenticator::Authenticator(Susi::SusiClient & susi, BSON::Value & config) :
      susi_{susi},
      config_{config} {
        loadFromFile();
    ...
    void Authenticator::setupDefaults(){
        ...
        BSON::Value basePattern = BSON::Object{
            {"topic","authenticator::(users|permissions)::.*"}
        };
        auto basePermission = Permission{
            .id = generateToken(),
            .pattern = Susi::Event{basePattern},
            .roles = std::vector<std::string>{"admin"}
        };
        addPermission(basePermission);

    }

    void Authenticator::saveToFile(){
        ...
        BSON::Value saveObject = BSON::Object{
            {"users",userArray},
            {"permissions",permissions}
        };
        std::string fileLocation = "/usr/share/susi/authenticator.json";
        if(config_["file"].isString()){
            fileLocation = config_["file"].getString();
        }
        std::ofstream file{fileLocation};
        file << saveObject.toJSON();
        file.close();
    }

    void Authenticator::loadFromFile(){
        std::string fileLocation = "/usr/share/susi/authenticator.json";
        if(config_["file"].isString()){
            fileLocation = config_["file"].getString();
        }
        ...
        auto obj = BSON::Value::fromJSON(fileStr);
        ...
        auto & permissions = obj["permissions"];
